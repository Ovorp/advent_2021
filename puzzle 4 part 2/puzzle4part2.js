...
function checkBoard(boards) {
  ...
  totalboards = totalboards.filter((board) => {
    return !checkRow(board) && !checkColumn(board);
  });

  if (totalboards.length === 1) {
